...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Root
    ref={ref}
    className={cn(
      "w-full -full [&>*:first-child]:border-t [&>*:first-child]:border-foreground",
      className,
    )}
    {...props}
  />
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between text-2xl py-4 transition-all [&[data-state=open]>svg]:rotate-180",
        className,
      )}
      {...props}
    >
